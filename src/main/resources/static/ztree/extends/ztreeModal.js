...
$.extend(ztreeModal.prototype,{
		...
		this.ztreeId = obj.dropDom.attr('data-ztreeId'),
	     temp = '<div class="form-init-drop '+direction+'">'+
		          '<div class="input-group">'+
		              '<input name="peopleAddress" type="text" class="form-control" id="input_'+this.ztreeId+'" readonly required>'+
	...
	bindEvent:function(){
		...
		obj.dropDom.on('click','.checkBtn',function(e){
            $("#input_" + obj.ztreeId).popover('destroy');
            $("#input_"+ obj.ztreeId).css("border","");
            $("#input_"+ obj.ztreeId).focus();
			e.stopPropagation();
